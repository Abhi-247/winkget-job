      ...
      <aside className={cn(
        ...
        <div className="px-5 py-4 border-b border-slate-800/50">
          <div className="flex items-center justify-between">
            <Link href="/admin/dashboard" className="flex items-center gap-2.5">
              <div className="w-8 h-8 bg-red-600 rounded-lg flex items-center justify-center text-white font-black text-sm shadow-md shadow-red-900/25">
                W
              </div>
              <div>
                ...
                <li key={item.href}>
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-red-950/30 text-white font-semibold border-l-2 border-red-500 pl-2.5 shadow-inner"
                  ...
                  >
                    <Icon 
                      ...
                      className={cn(
                        ...
                        isActive ? "text-red-500 scale-110" : "text-gray-500 group-hover:text-gray-300"
                      )} 
                    />
                    <span className="flex-1">{item.label}</span>
                    {isActive && <ChevronRight size={12} className="text-red-500 animate-pulse" />}
                  </Link>
                </li>
              );
            })}
        ...
        <div className="p-3 border-t border-slate-800/50 bg-[#06090f]">
          <div className="flex items-center gap-3 bg-slate-900/30 border border-slate-800/20 rounded-xl p-2.5 mb-2">
            <Avatar name={user?.name || "Admin"} src={user?.image} size="sm" className="ring-1 ring-slate-700/30" />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-white truncate">{user?.name || "Admin"}</p>
              <p className="text-[10px] text-red-500 font-medium truncate">Administrator</p>
            </div>
          </div>
          <button
