      ...
      {cards.map((card) => {
        ...
        return (
          <Link key={card.key} href={card.href} className="block group">
            <div className={`${card.bg} rounded-xl p-2.5 sm:p-4 border border-white group-hover:shadow-sm transition-shadow min-w-0`}>
              <div className={`${card.iconBg} w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center mb-2 sm:mb-3`}>
                <Icon size={14} className={card.iconColor} />
              </div>
              <div className={`text-lg sm:text-2xl font-bold ${card.valueColor} mb-0.5 truncate`}>
                {stats[card.key] ?? 0}
              </div>
              <div className="text-[10px] sm:text-xs text-gray-500 font-medium leading-tight truncate">{card.label}</div>
            </div>
          </Link>
        );
      })}
